...
int main() {
   ...
   // std::cout << "[INFO] T1: " << T1 << ", T2: " << T2 << std::endl;
    ...
    for (int i = 0; i < SIMULATIONS; i++) {
        ...
        size_t served_customers = customer.get_served_customers();  
        stat.save(static_cast<real_t>(served_customers) / HORIZON);

        //std::cout << "[Simulation] Iteration " << i + 1 << " Customers Served: " << served_customers << std::endl;

        customer.reset();  //  Resetto il cliente per la prossima simulazione
    }
